...
def vis_flow(pc1, pc2=None, sf=None, sf_color = None, color=None, seg=False,
    ...
    if seg and sf is not None:
        labels = utils.flow_segmentation(pc1, sf)
...
def vis_seg(pc):
    ...
    labels = utils.graph_segmentation(pc, threshold=.25)
...
def vis_boxes_from_params(pc1, pc2, ego_transform, boxes, box_transform):
    ...
    transformed_boxes = utils.transform_boxes(boxes, ego_transform_r)
...
def vis_boxes(points, boxes, transform = None, pc2 = None, savefig = False, filename = 'box_vis.png', view=None):
    mlab.options.offscreen = savefig
    num_boxes = len(boxes)
    ...
    for n, box in enumerate(boxes):
        ...
        if params == 7:
            draw_box(corners, fig, (n+2)/(num_boxes+1), (n+2)/(num_boxes+1))
        elif params == 8:
            draw_box(corners, fig, (n + 2) / (num_boxes + 1), c)
    ...
    mlab.points3d(x1, y1, z1, mode="point", color=(1, 1, 1), figure = fig)
